...
#cat 127.0.0.1 lists
cat /root/adblocklists/malwaredomainlist.txt | grep '^127\.0\.0\.1' | awk '{print "local-zone: \""$2"\" always_nxdomain"}' >> /root/adblocklists/unboundadblock_temp.list
cat /root/adblocklists/hostsfileorg.txt | grep '^127\.0\.0\.1' | awk '{print "local-zone: \""$2"\" always_nxdomain"}' >> /root/adblocklists/unboundadblock_temp.list
